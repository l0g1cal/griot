...
- **[tacit/](tacit/)** — the Tacit prototype: the knowledge graph (seed JSON, ready to feed the UI) and the rules generator that emits `CLAUDE.md` + `.cursor/rules` into the appeals repo.
...
| 3 | **Genuine accident.** A null check on `normalizeRef()`'s result that can never fire (`String.replace` always returns a string). No commit, PR, or comment explains it. | `appeals-service/src/legacy/ticketAdapter.ts` (`ticketToCase`) | None — that's the point. Tacit raises open question **q-001**, routed to Dan Okafor via git blame. |
...
npm run generate       # graph → appeals-service/CLAUDE.md + .cursor/rules/tacit-domain.mdc
...
1. **Without Tacit:** `rm appeals-service/CLAUDE.md && rm -rf appeals-service/.cursor`. Open Claude Code / Cursor in `appeals-service/` and prompt:
...
2. **With Tacit:** `cd tacit && npm run reset` (regenerates `CLAUDE.md`), then the **same prompt**.
